...
public class ContextInfoManager implements XUIManagerBase {
    ...
    public void handleAccelerationEvent(float accelerationValue) {
        ...
        handler.sendMessage(handler.obtainMessage(1, Float.valueOf(accelerationValue)));
    }

    public void handleAngularVelocityEvent(float angularVelocityValue) {
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(2, Float.valueOf(angularVelocityValue)));
    }
    ...
    public void handleNavigationInfo(String navInfo) {
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(12, navInfo));
    }

    public void handleNavigationEnableEvent(boolean enable) {
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(13, Boolean.valueOf(enable)));
    }

    public void handleMsgEvent(int msgType) {
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(14, Integer.valueOf(msgType)));
    }
    ...
    public void handleWeatherInfo(String weatherInfo) {
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(15, weatherInfo));
    }

    public void handleGearStatus(int gear) {
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(MSG_CONTEXTINFO_CAR_GEAR, Integer.valueOf(gear)));
    }

    public void handleCarSpeed(float speed) {
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(16, Float.valueOf(speed)));
    }

    public void handleIGStatus(int status) {
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(17, Integer.valueOf(status)));
    }
    ...
    public void handleAvpWifi(int status) {
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(20, Integer.valueOf(status)));
    }

    public void handleRemoteCommand(int cmd) {
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(MSG_CONTEXTINFO_REMOTE_CMD, Integer.valueOf(cmd)));
    }
    ...
    public void handleBeltStatus(int status) {
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(MSG_CONTEXTINFO_BELT_STATUS, Integer.valueOf(status)));
    }

    public void handlePowerAction(int poweraction) {
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(MSG_CONTEXTINFO_POWER_ACTION, Integer.valueOf(poweraction)));
    }

    public void handleChargingGunStatus(int status) {
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(MSG_CONTEXTINFO_CHARGING_GUN, Integer.valueOf(status)));
    }

    public void handleDeviceChargeStatus(int status) {
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(MSG_CONTEXTINFO_DEVICE_CHARGE, Integer.valueOf(status)));
    }

    public void handleDriverSeatStatus(int status) {
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(MSG_CONTEXTINFO_DRIVER_SEAT, Integer.valueOf(status)));
    }
    ...
    public void handlePsdMoto(int status) {
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(MSG_CONTEXTINFO_PSD_MOTO, Integer.valueOf(status)));
    }

    public void handlePowerOffCount(int cnt) {
        Handler handler = this.mHandler;
        handler.sendMessage(handler.obtainMessage(32, Integer.valueOf(cnt)));
    }
    ...
    public void onXUIConnected(IBinder service) {
        ...
        this.mService = IContextInfo.Stub.asInterface(service);
        ContextInfoEventListenerToService contextInfoEventListenerToService = this.mListenerToService;
        if (contextInfoEventListenerToService != null) {
            try {
                this.mService.registerListeners(contextInfoEventListenerToService, 1);
            } catch (RemoteException e) {
