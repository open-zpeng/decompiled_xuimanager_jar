...
import com.xiaopeng.xuimanager.deviceshare.data.ProtocolInfo;
...
public class DeviceShareManager implements XUIManagerBase {
    ...
    public int[] getSupportedProtocols() {
        ParcelableContainer dataOut = new ParcelableContainer();
        int ret = this.pipeBusClient.ioControlWithParcelable(MODULE_NAME, DeviceShareConst.CMD_GET_SUPPORTED_PROTOCOLS, null, dataOut);
        if (ret != 0) {
            return null;
        }
        int[] protocols = dataOut.getIntArray();
        return protocols;
    }

    public ProtocolInfo getProtocolInfo(int protocol) {
        ParcelableContainer dataIn = new ParcelableContainer();
        ParcelableContainer dataOut = new ParcelableContainer();
        dataIn.setIntArray(new int[]{protocol});
        int ret = this.pipeBusClient.ioControlWithParcelable(MODULE_NAME, DeviceShareConst.CMD_GET_PROTOCOL_INFO, dataIn, dataOut);
        if (ret == 0) {
            Parcelable[] parcelables = dataOut.getParcelableArray();
            if (parcelables == null) {
                LogUtil.d(TAG, "getProtocolInfo null");
                return null;
            }
            try {
                ProtocolInfo protocolInfo = (ProtocolInfo) parcelables[0];
                return protocolInfo;
            } catch (Exception e) {
                String str = TAG;
                LogUtil.w(str, "getProtocolInfo e=" + e);
                return null;
            }
        }
        String str2 = TAG;
        LogUtil.w(str2, "getProtocolInfo,ret=" + ret);
        return null;
    }
    ...
    public int startApp(ShareAppInfo appInfo) {
        ...
        int ret = this.pipeBusClient.ioControlWithParcelable(MODULE_NAME, "startApp", dataIn, null);
        return ret;
    }
}
